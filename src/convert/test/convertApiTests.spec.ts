...
import {
  ...
  queryDate2018,
  queryDateIsNumber,
  queryDateIsNotISO8601v1,
  queryDateIsNotISO8601v2,
  queryDateIsNotISO8601v3,
  queryDateIsNotISO8601v4,
} from "./testQueries";
...
describe("API Tests: convert", () => {
  ...
  it("should fail: date is before 01/01/2010", async () => {
    ...
    expect(res.body.message[0]).toBe(
      `${DATE_MUST_BE_BETWEEN_2010_AND_PRESENT}. input: ${queryDate1997.date}`
    );
  });
  ...
  it("should fail: date is not ISO8601 v1", async () => {
    ...
    expect(res.body.message.length).toBe(1);
    expect(res.body.message[0]).toBe(
      `${DATE_MUST_BE_ISO8601}. input: ${queryDateIsNotISO8601v1.date}`
    );
  });
  ...
  it("should fail: date is not ISO8601 v2", async () => {
    ...
    expect(resultBody.message[0]).toBe(
      `${DATE_MUST_BE_ISO8601}. input: ${queryDateIsNotISO8601v2.date}`
    );
  });
  ...
  it("should fail: date is not ISO8601 v3", async () => {
    ...
    expect(res.body.message.length).toBe(1);
    expect(res.body.message[0]).toBe(
      `${DATE_MUST_BE_ISO8601}. input: ${queryDateIsNotISO8601v3.date}`
    );
  });
  ...
  it("should fail: date is not ISO8601 v4", async () => {
    ...
    expect(res.body.message.length).toBe(1);
    expect(res.body.message[0]).toBe(
      `${DATE_MUST_BE_ISO8601}. input: ${queryDateIsNotISO8601v4.date}`
    );
  });
  ...
  it("should fail: date is a future date", async () => {
    ...
    expect(res.body.message.length).toBe(1);
    expect(res.body.message[0]).toBe(
      `date must be between 1/1/2010 and present . input: ${queryDateTomorrow.date}`
    );
  });
